...
function ProjectCard({ project }: { project: Project }) {
  ...
  const handleSave = async () => {
    ...
    if (editName !== project.name) {
      try {
        await updateProject(project.id, editName, project.description);
...
function AppContent() {
  ...
  useEffect(() => {
    if (currentDiagram) {
      loadCanonical(currentDiagram.canonical_json);
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {currentDiagram && (
          <>
            ...
            {/* Added ValidationPanel */}
            <div className="w-[300px] border-l border-gray-300 bg-white flex flex-col h-full overflow-hidden shrink-0">
              <div className="flex-1 relative h-1/2 overflow-y-auto border-b">
                <ProblemsPanel />
              </div>

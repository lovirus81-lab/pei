"""규칙 기반 검증 엔진 — 검증 계층
validator.py에서 이전. NodeType/EdgeType import 추가로 런타임 크래시 수정.
"""
...
from app.domain.models.enums import NodeType, EdgeType  # 런타임 크래시 수정 — 반드시 필요
from app.validation.rule_models import ValidationReport, Violation

logger = logging.getLogger(__name__)


def _get_downstream_nodes(
    canonical: DiagramCanonical, node_id: str, max_distance: int
) -> List[CanonicalNode]:
    visited: set[str] = set()
    queue: deque[tuple[str, int]] = deque([(node_id, 0)])
    found: List[CanonicalNode] = []
...
def _get_upstream_nodes(
    canonical: DiagramCanonical, node_id: str, max_distance: int = 1
) -> List[CanonicalNode]:
    visited: set[str] = set()
    queue: deque[tuple[str, int]] = deque([(node_id, 0)])
    found: List[CanonicalNode] = []
        ...
        if match == "node":
            ...
            for node in canonical.nodes:
                ...
                if "has_property" in check_dict:
                    prop_name = (
                        list(check_dict["has_property"].keys())[0]
                        if isinstance(check_dict["has_property"], dict)
                        else check_dict["has_property"]
                    )
                ...
                if check_dict.get("has_bypass"):
                    ...
                    bypass_found = False
                    for u in upstream:
                        u_downstreams: List[CanonicalNode] = []
                        for edge in canonical.edges_from(u.id):
                            if edge.to_node != node.id:
                                u_downstreams.extend(_get_downstream_nodes(canonical, edge.to_node, 3))
                                maybe = canonical.node_by_id(edge.to_node)
                                if maybe:
                                    u_downstreams.append(maybe)
                        u_down_ids = {n.id for n in u_downstreams}
                ...
                if check_dict.get("connected_node"):
                    ...
                    for edge in canonical.edges_from(node.id) + canonical.edges_to(node.id):
                        ...
                        target_node = canonical.node_by_id(target_id)
                        if target_node:
                            if req_type and target_node.type.value != req_type:
                                continue
                            if req_subtype and target_node.subtype != req_subtype:
                                continue
                            found = True
                            break
                    if not found:
            ...
            for edge in canonical.edges:
                ...
                # line_number_matches_pattern
                if "line_number_matches_pattern" in check_dict:
                    pattern = check_dict["line_number_matches_pattern"]
                    logger.debug("[%s] pattern=%s, line_number=%s", rule.code, pattern, edge.line_number)
